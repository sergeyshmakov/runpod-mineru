...
This is the community pattern for fixed multi-model workers (worker-comfyui, worker-faster_whisper, runpod-worker-real-esrgan all do the same). Trade-off: the image is ~4 GB larger, but RunPod caches images regionally so pulls are fast after the first.
